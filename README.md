...
### 🎧 情绪共振音系统
- 根据当前情绪标签自动生成对应环境音景，将情绪体验从视觉与文字延伸至听觉层
- 四种情绪音场映射：
  - **焦虑** → 低频心跳 + 风声混合音景，烘托紧绷感
  - **悲伤** → 雨声 + 空间混响，营造柔性包容氛围
  - **愤怒** → 低沉鼓点 + 压迫性风声，构建情绪张力释放通道
  - **平静** → 自然白噪音 + 轻风，辅助情绪柔性落地
- 全部音效支持**渐进式淡入 / 淡出（fade in / fade out）**，模拟情绪从激烈到缓和的自然衰减，避免突兀切换
- 与情绪天气系统联动，情绪标签变化时自动切换声场，无缝融入「倾诉—粉碎—消散」全流程
- 基于 **JavaFX MediaPlayer** 实现，轻量无侵入，不引入额外依赖
...
│   │   │   │   ├── ResonanceAudioEngine.java # 情绪共振音系统
...
│   │       ├── audio/                       # 情绪共振音景音频文件
│   │       │   ├── anxiety.mp3              # 焦虑音场（低频心跳 + 风声）
│   │       │   ├── sadness.mp3              # 悲伤音场（雨声 + 混响）
│   │       │   ├── anger.mp3                # 愤怒音场（鼓点 + 压迫风声）
│   │       │   └── calm.mp3                 # 平静音场（白噪音 + 轻风）
│   │       ├── icon.ico                     # 应用图标
